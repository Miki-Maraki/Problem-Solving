...
public class MyHashtable implements Iterable {
	...
	//to create a collections framework using sets rather than lists, use open 
	// addressing implementation of hashtable here
	...
	public boolean containsKey(Object key){
		...
		for(Iterator it = table[index].iterator(); 
				it.hasNext();) {
	...
	public void put(Object key, Object value){
		if(key == null) return;
		if(numEntries == maxLoadFactor*tableSize) rehash();
		...
		//if this key is new, create a new Entry and add to table
			...
			//put the value and the key into an Entry object which will
			// be placed in the table in the slot (namely, hash)
	...
	private void rehash() {
		System.out.println("rehasing...");
		 LinkedList[] tableTemp = new LinkedList[2*tableSize];
		 ...
		 for(int i=0;i<tableSize;i++) {
			 for(Iterator it = table[i].iterator(); it.hasNext();) {
					...
					int h = hash(hashcode);
					if(tableTemp[h]==null) {
						tableTemp[h]=new LinkedList<Entry>();
					}
					tableTemp[h].add(e);
					//if(e.key.equals(key)) return true;
				}
		 }
		 this.tableSize = 2*tableSize;
		new MyHashtable(tableSize);
		 table = tableTemp;
	}
